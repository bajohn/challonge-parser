...
const cleanNames = async (participants: iParticipant[]) => {
  const ret = participants.reduce<{ [index: number]: string[] }>((lv, cv) => {
    const id = cv.participant.id;
    // TODO -test this!
    // Doubles tournaments have a special character
    // "/", which indicates we need to add results
    // for two cleaned names
    const rawName = cv.participant.name;
    const names =
      rawName.indexOf("/") > -1
        ? rawName.split("/").map((name) => cleanName(name))
        : [cleanName(cv.participant.name)];
