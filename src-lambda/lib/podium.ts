...
export const parsePodium = (
  statStoreIn: iStatStore,
  participants: iParticipant[],
  cleanedNameMap: cleanedNameMap,
  tourney: iTournament
) => {
  ...
  for (const el of participants) {
    ...
    const cleanNames = cleanedNameMap[participant.id];
    // NEXT TODO - test this in a unit test
    // AND make sure this works for doubles!
...
export const prettifyPodium = (statStoreIn: iStatStore) => {
  ...
  for (const podiumName of podiumNames) {
    ...
    playersArr.sort((a, b) => (a.split(":")[1] > b.split(":")[1] ? -1 : 1));
